...
for er_idx = 1 : 1 : length(stratification.er)
    ...
    [k_comp, ~, kx, ky, kz] ...
        = wave_vector_multi_freq(stratification.er(er_idx), wave.k0, ...
        sph_grid);
    krho = sqrt(kx .^ 2 + ky .^ 2);
    ...
    [vte, ite, vtm, itm] ...
        ...
        'SemiInfiniteSuperstrate', stratification.h, ...
        stratification.er(er_idx));
            
    %% SPECTRAL GREEN'S FUNCTIONS
    SGF = spectral_gf_multi_freq(stratification.er(er_idx), wave.k0, ...
        kx, ky, vtm, vte, itm, ite, 'E', 'M');
            
    %% ELECTRIC FIELD
    k = wave.k0 * sqrt(stratification.er(er_idx));
    E = farfield_multi_freq(k, R, sph_grid, kz, z, SGF, Mx);
    if er_idx == length(stratification.er)
        Es(:, :, 1) = sqrt( abs(E(:, :, 1, 1)) .^ 2 ...
            + abs(E(:, :, 2, 1)) .^ 2 + abs(E(:, :, 3, 1)) .^ 2);
        Es(:, :, 2) = sqrt( abs(E(:, :, 1, 51)) .^ 2 ...
            + abs(E(:, :, 2, 51)) .^ 2 + abs(E(:, :, 3, 51)) .^ 2);
        Es(:, :, 3) = sqrt( abs(E(:, :, 1, end)) .^ 2 ...
            + abs(E(:, :, 2, end)) .^ 2 + abs(E(:, :, 3, end)) .^ 2);
    end
            
    %% DIRECTIVITY
    dir_broadside(er_idx, :) ...
        = broadside_directivity(stratification.er(er_idx), E, sph_grid, R);
...
sgtitle(['E Far-Field @ Superstrate, h = ' num2str(stratification.h ...
    * 1e3) ' mm, \epsilon_{r} = ' num2str(stratification.er(end))], ...
